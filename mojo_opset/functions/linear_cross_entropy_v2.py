...
class LinearCrossEntropyV2Function(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        inputs,
...
def _run(
    ...
    *,
    ...
    loss, zloss, accuracy, _lse, _lse_abs = LinearCrossEntropyV2Function.apply(
        inputs,
        weight,
        labels,
        ignore_index,
...
def linear_cross_entropy_v2(
    ...
    *,
    ...
    """Ext-derived chunked CE without z-loss; not the original mojo CE interface.

    Returns loss, or (loss, accuracy) when calc_acc=True.
    For original mojo's bias, class weights and smoothing, use linear_cross_entropy.
    """
...
def linear_cross_entropy_v2_and_zloss(
    inputs: torch.Tensor,
    weight: torch.Tensor,
    labels: torch.Tensor,
    z_loss_weight: float,
    *,
    ...
    """Ext-derived chunked CE with z_loss_weight * logsumexp(abs(logits))**2.
    ...
    without their own gradients. Original mojo's linear_cross_entropy instead
    uses ordinary logsumexp(logits)**2.
    """
