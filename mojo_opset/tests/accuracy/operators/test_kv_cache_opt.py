...
from mojo_opset.backends.ttx.kernels.npu.kv_cache_opt import store_paged_kv_opt_impl
...
def test_store_paged_kv_opt(batch_size, kv_heads, head_dim, block_size, context_kv_lens_val, q_lens_val):
    ...
    k_cache_opt = k_cache_ref.clone()
    v_cache_opt = v_cache_ref.clone()
    ...
    k_cache_opt, v_cache_opt = store_paged_kv_opt_impl(
        key_states,
        value_states,
        k_cache_opt,
        v_cache_opt,
    ...
    assert_close(k_cache_opt, k_cache_ref)
    assert_close(v_cache_opt, v_cache_ref)


@auto_switch_platform()
@bypass_not_implemented
def test_store_paged_kv_opt_bucket_padded_varlen():
    real_batch_size = 4
    bucket_batch_size = 6
    token_bucket_size = 8
    kv_heads = 2
    ...
    for batch_id in range(real_batch_size):
        ...
        assert_close(
            k_cache_opt[phys_block, :, block_offset : block_offset + 1, :],
            k_cache_ref[phys_block, :, block_offset : block_offset + 1, :],
        )
        assert_close(
            v_cache_opt[phys_block, :, block_offset : block_offset + 1, :],
