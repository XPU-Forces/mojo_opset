...
def test_rotary_embedding(bs, seqlen, rope_dim, mode):
    ...
    if mode == "padding_prefill":
        ...
        torch.testing.assert_close(
            rot_pos_emb_ref(x, cu_seqlens_q=cu_seqlens, total_seq_lens=kv_lens),
            rot_pos_emb(x, cu_seqlens_q=cu_seqlens, total_seq_lens=kv_lens),
            atol=1e-5,
            rtol=1e-5,
        )
        rot_pos_emb.forward_diff_with(
...
def test_apply_rope(bs, seqlen, q_heads, k_heads, head_first, head_dim, rope_percentage, mode, dtype):
    ...
    if mode == "padding_prefill_pos3d":
        ...
        cos, sin = rot_pos_emb(x, cu_seqlens_q=cu_seqlens, total_seq_lens=kv_lens)
        if head_first:
            q = q.transpose(0, 1)
            k = k.transpose(0, 1)
    elif mode == "decode":
