...
class MojoFlashAttnFunction(MojoFuncBase):
    ...
    def forward_ref(
        ...
        for q0 in range(0, Sq, block_q_len):
            ...
            y_block = n / (l[..., None] + eps) # [B, G, Nk, ql, D]
    ...
    def backward_ref(ctx, grad_y):
        ...
        grad_y = grad_y.permute(0, 2, 1, 3).reshape(B, G, Nk, Sq, Dv) # [B, S, Nq, D] -> [B, G, Nk, S, D]

        for q0 in range(0, Sq, block_q_len):
            ql = min(block_q_len, Sq - q0)
            q_block = q[:, :, :, q0:q0+ql, :] # [B, G, Nk, ql, D]
            grad_y_block = grad_y[:, :, :, q0:q0+ql, :] # [B, G, Nk, ql, Dv]
            ...
            for k0 in range(0, Sk, block_k_len):
                ...
                exp_m_mnew = torch.exp(m - m_new) # [B, G, Nk, ql]
                exp_s_mnew = torch.exp(scores - m_new[..., None]) # [B, G, Nk, ql] -> [B, G, Nk, ql, kl]
                l = exp_m_mnew * l + exp_s_mnew.sum(dim=-1) # [B, G, Nk, ql, kl] -> [B, G, Nk, ql]

                m = m_new
                cache_scores_blocks.append((k0, kl, m_new))
        ...
        grad_q = grad_q.reshape(B, Nq, Sq, D).permute(0, 2, 1, 3)
