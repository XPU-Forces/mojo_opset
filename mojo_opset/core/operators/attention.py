...
class MojoPagedDecodeGQA(MojoOperator):
    ...
    def forward(
        ...
        """
        Batched paged decode attention with grouped query heads (GQA) using a blocked KV cache.

        Args:
            query (torch.Tensor): Query tensor of shape (B, Hq, D), where B is batch size,
                Hq is the number of query heads, and D is head dimension.
            ...
            seqlens (torch.Tensor): Per-batch sequence lengths of shape (B,), used for causal masking.
            block_tables (torch.Tensor): Block table of shape (B, num_blocks), mapping logical
                block indices to physical block IDs in the KV cache.
            softmax_scale (Optional[float], default=None): Scale applied to attention logits;
                defaults to 1/sqrt(D) when None.

        Returns:
            torch.Tensor: Attention output of shape (B, Hq, D).

        Notes:
            - If Hq > Hkv, K/V heads are repeated (repeat_interleave) to match query heads.
            - Causal masking uses `seqlens` to mask positions beyond each batch's actual length.
            - Softmax is computed in float32 for numerical stability and cast back to the input dtype.
            - KV reconstruction stitches contiguous per-batch sequences using `block_tables`.
        """
...
class MojoPagedPrefillGQA(MojoOperator):
    def __init__(
        ...
        """
        Initialize the Paged Prefill GQA attention operator.
    ...
    def forward(
        ...
        Args:
            ...
            key_cache (torch.Tensor): Key cache of shape (N_blocks, Hkv, block_size, D).
            value_cache (torch.Tensor): Value cache of shape (N_blocks, Hkv, block_size, D).
            cu_seq_lens (torch.Tensor): Cumulative query lengths, shape (B+1,);
                `cu_seq_lens[i]` is the start offset for batch i; `cu_seq_lens[-1] == T`.
        ...
        num_total_blocks, num_kv_heads, block_size, _ = key_cache.shape
        if softmax_scale is None:
            softmax_scale = 1.0 / math.sqrt(head_dim)
        ...
        for i in range(batch_size):
            ...
            start_loc = cu_seq_lens[i].item()
            end_loc = cu_seq_lens[i + 1].item()
