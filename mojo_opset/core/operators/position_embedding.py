...
class MojoRotaryEmbedding(MojoOperator):
    def __init__(
        self, rope_theta, rope_dim, attention_scaling: float = 1.0, init_max_length: Optional[int] = None, **kwargs
    ):
        ...
        inv_freq = 1.0 / (
            self.rope_theta
            ** (
                torch.arange(0, rope_dim, 2, dtype=torch.float32, device=self.tensor_factory_kwargs.get("device"))
                / rope_dim
            )
        )
    ...
    def forward(
        ...
        if cu_seqlens_q is not None:
            ...
            assert position_ids.shape == x.shape[:-1], (
                "position_ids must have the same shape as x except the hidden dimension"
            )
        else:
...
class MojoApplyRoPE(MojoOperator):
    ...
    def forward(
        ...
        if q.ndim == 3:
            assert cos.ndim == 2, (
                "rotary position embedding (cos/sin) must be of shape [num_tokens, rope_dim] for varlen prefill or decode"
            )
