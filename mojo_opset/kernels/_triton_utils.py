...
def rope_head_first(cos_ndim: int, unsqueeze_dim: int) -> bool:
    """Translate the public RoPE broadcast dimension to the internal layout flag."""
    if isinstance(unsqueeze_dim, bool):
        raise TypeError("unsqueeze_dim must be an integer dimension, not a bool layout flag.")
    output_ndim = cos_ndim + 1
    if unsqueeze_dim < 0:
        unsqueeze_dim += output_ndim

    head_first_dim = cos_ndim - 2
    sequence_first_dim = cos_ndim - 1
    if unsqueeze_dim == head_first_dim:
        return True
    if unsqueeze_dim == sequence_first_dim:
        return False
    raise ValueError(
        f"unsqueeze_dim must be {head_first_dim} (head-first) or "
        f"{sequence_first_dim} (sequence-first) for {cos_ndim}D cos/sin tensors."
    )
...
def tensor_device_guard_for_triton_kernel(path, name, device="npu"):
    from mojo_opset.utils.env import get_bool_env
