...
@torch.library.custom_op("mojo_ilu_triton::group_rms_norm_single_infer", mutates_args=())
def group_rms_norm_single_infer(
    x: torch.Tensor, weight: Optional[torch.Tensor], eps: float,
) -> torch.Tensor:
    return _rmsnorm_fwd_single(x.contiguous(), None if weight is None else weight.contiguous(),
