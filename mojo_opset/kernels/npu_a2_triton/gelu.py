...
@torch.library.custom_op("mojo_npu_triton_a2::gelu_fwd", mutates_args=())
def gelu_fwd(x: torch.Tensor, approximate: str = "tanh") -> torch.Tensor:
    if approximate != "tanh":
        raise NotImplementedError("Triton GELU supports approximate='tanh' only")
    return gelu_fwd_impl(x.contiguous())


@gelu_fwd.register_fake
def _gelu_fwd_fake(x: torch.Tensor, approximate: str = "tanh") -> torch.Tensor:
    if approximate != "tanh":
        raise NotImplementedError("Triton GELU supports approximate='tanh' only")
    return torch.empty_like(x, memory_format=torch.contiguous_format)


@torch.library.custom_op("mojo_npu_triton_a2::gelu_bwd", mutates_args=())
def gelu_bwd(grad_output: torch.Tensor, x: torch.Tensor, approximate: str = "tanh") -> torch.Tensor:
    if approximate != "tanh":
        raise NotImplementedError("Triton GELU supports approximate='tanh' only")
    return gelu_bwd_impl(grad_output.contiguous(), x.contiguous())


@gelu_bwd.register_fake
def _gelu_bwd_fake(grad_output: torch.Tensor, x: torch.Tensor, approximate: str = "tanh") -> torch.Tensor:
    if approximate != "tanh":
        raise NotImplementedError("Triton GELU supports approximate='tanh' only")
    del grad_output
    return torch.empty_like(x, memory_format=torch.contiguous_format)
