...
@torch.library.custom_op("mojo_npu_triton_a2::swa_infer", mutates_args=())
def swa_infer_fwd(
    q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
    cu_q_lens: torch.Tensor, cu_k_lens: torch.Tensor,
    is_causal: bool, local_window_size: Optional[int], global_window_size: Optional[int],
    softmax_scale: float, gqa_interleave: bool,
) -> torch.Tensor:
    if is_causal and local_window_size is None and global_window_size is None:
        local_window_size = k.shape[0]
    # Triton still compiles the body of the zero-trip global-window loop.
    # Pass a numeric bound so its mask arithmetic never receives None.
    if global_window_size is None:
        global_window_size = 0
    return swa_infer_impl(q, k, v, cu_q_lens, cu_k_lens, is_causal, local_window_size,
                          global_window_size, softmax_scale, gqa_interleave)


@swa_infer_fwd.register_fake
def _swa_infer_fake(q, k, v, cu_q_lens, cu_k_lens, is_causal, local_window_size,
                    global_window_size, softmax_scale, gqa_interleave):
...
def _swa_bwd_dkdv_kernel(
    ...
    for b_id in range(bsz):
        ...
        if IS_CAUSAL:
            if LOCAL_WINDOW is None and GLOBAL_WINDOW is not None:
                # Non-global keys have no attending queries. Leave their initialized
                # gradients at zero instead of entering a zero-matmul task.
                num_kv_chunks = min(num_kv_chunks, tl.cdiv(GLOBAL_WINDOW, BLOCK_N))
        balance_ratio = 1
...
@torch.library.custom_op("mojo_npu_triton_a2::swa_preprocess", mutates_args=())
def swa_preprocess(do: torch.Tensor, o: torch.Tensor) -> torch.Tensor:
    tot_q_toks, num_q_heads, head_dim = o.shape
    delta = torch.zeros((num_q_heads, tot_q_toks), dtype=torch.float32, device=o.device)
    o = o.contiguous()
    do = do.contiguous()
...
@swa_preprocess.register_fake
def _swa_preprocess_fake(do, o):
    return o.new_empty((o.shape[1], o.shape[0]), dtype=torch.float32)


@torch.library.custom_op("mojo_npu_triton_a2::swa_dkdv", mutates_args=())
def swa_dkdv(
    do: torch.Tensor,
    delta: torch.Tensor,
    q: torch.Tensor,
...
@torch.library.custom_op("mojo_npu_triton_a2::swa_dq", mutates_args=())
def swa_dq(
    do: torch.Tensor,
    delta: torch.Tensor,
    q: torch.Tensor,
...
@torch.library.custom_op(
    "mojo_npu_triton_a2::swa_fwd",
    mutates_args=(),
    # PyTorch 2.7 cannot infer optional tensors in a tuple return annotation.
    schema="(Tensor q, Tensor k, Tensor v, Tensor cu_q_lens, Tensor cu_k_lens, "
           "bool is_causal, SymInt? local_window_size, SymInt? global_window_size, "
           "float softmax_scale, bool gqa_interleave, bool output_f32=False) -> (Tensor, Tensor, Tensor?)",
)
def swa_fwd(
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    # Do not return O twice: custom-op outputs cannot alias each other.
    return result if output_f32 else (result[0], result[1], None)


@swa_fwd.register_fake
def _swa_fwd_fake(
    q,
    k,
    v,
    ...
    del k, v, cu_q_lens, cu_k_lens, is_causal
    del local_window_size, global_window_size, softmax_scale, gqa_interleave
    return (
        torch.empty_like(q, memory_format=torch.contiguous_format),
        q.new_empty((q.shape[1], q.shape[0]), dtype=torch.float32),
        q.new_empty(q.shape, dtype=torch.float32) if output_f32 else None,
    )
...
def flash_attention_infer_fwd(q, k, v, attention_mask, cu_q_lens, cu_k_lens,
                              max_q_len, max_k_len, dropout_p, softmax_scale, causal, deterministic):
    if attention_mask is not None or dropout_p != 0.0:
        raise NotImplementedError("NPU Triton inference requires dropout_p=0 and attention_mask=None")
    return swa_infer_fwd(q, k, v, cu_q_lens, cu_k_lens, causal,
                         max_k_len if causal else None, None, softmax_scale, False)
...
def flash_attention_fwd(
    ...
    del max_q_len, deterministic
    if attention_mask is not None:
        raise NotImplementedError("NPU Triton flash_attention does not support attention_mask")
    if dropout_p != 0.0:
        raise NotImplementedError("NPU Triton flash_attention currently supports dropout_p=0 only")
    local_window_size = max_k_len if causal else None
    return swa_fwd(
        q,
        k,
        v,
...
@torch.library.custom_op("mojo_npu_triton_a2::paged_prefill_swa_infer_fwd", mutates_args=())
def paged_prefill_swa_infer_fwd(query: torch.Tensor, key_cache: torch.Tensor, value_cache: torch.Tensor,
    cu_q_lens: torch.Tensor, block_table: torch.Tensor, softmax_scale: Optional[float],
    cu_total_seq_lens: Optional[torch.Tensor], max_q_len: Optional[int], max_total_seq_len: Optional[int],
    is_causal: bool, gqa_layout: str, global_window_size: Optional[int],
    local_window_size: Optional[int]) -> torch.Tensor:
    cu_k = cu_q_lens if cu_total_seq_lens is None else cu_total_seq_lens
    lengths = cu_k[1:] - cu_k[:-1]
    return swa_paged_prefill_impl(query, key_cache, value_cache, cu_q_lens, lengths, block_table,
        is_causal, local_window_size, global_window_size, softmax_scale, gqa_layout == "ABAB")


@paged_prefill_swa_infer_fwd.register_fake
def _paged_swa_prefill_fake(query, key_cache, value_cache, cu_q_lens, block_table, softmax_scale,
    cu_total_seq_lens, max_q_len, max_total_seq_len, is_causal, gqa_layout, global_window_size, local_window_size):
    return torch.empty_like(query, memory_format=torch.contiguous_format)


@torch.library.custom_op("mojo_npu_triton_a2::paged_decode_swa_infer_fwd", mutates_args=())
def paged_decode_swa_infer_fwd(query: torch.Tensor, key_cache: torch.Tensor, value_cache: torch.Tensor,
    total_seq_lens: torch.Tensor, block_table: torch.Tensor, softmax_scale: Optional[float],
    max_total_seq_len: Optional[int], is_causal: bool, gqa_layout: str,
    global_window_size: Optional[int], local_window_size: Optional[int]) -> torch.Tensor:
    return swa_paged_decode_impl(query, key_cache, value_cache, total_seq_lens, block_table,
        local_window_size, global_window_size, gqa_layout == "ABAB", softmax_scale)


@paged_decode_swa_infer_fwd.register_fake
def _paged_swa_decode_fake(query, key_cache, value_cache, total_seq_lens, block_table, softmax_scale,
    max_total_seq_len, is_causal, gqa_layout, global_window_size, local_window_size):
    return torch.empty_like(query, memory_format=torch.contiguous_format)
