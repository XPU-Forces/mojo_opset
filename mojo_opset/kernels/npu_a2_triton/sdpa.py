...
@torch.library.custom_op("mojo_npu_triton_a2::sdpa_infer_fwd", mutates_args=())
def sdpa_infer_fwd(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                   mask: Optional[torch.Tensor], scale: float, enable_gqa: bool) -> torch.Tensor:
    # Every inherited autotune candidate must be safe, including BLOCK_N=512.
    if query.shape[2] % 512:
        raise NotImplementedError("Triton sdpa_infer requires sequence length divisible by 512")
    return sdpa_infer_impl(query, key, value, mask, scale, enable_gqa)


@sdpa_infer_fwd.register_fake
def _sdpa_fake(query, key, value, mask, scale, enable_gqa):
    if query.shape[2] % 512:
        raise NotImplementedError("Triton sdpa_infer requires sequence length divisible by 512")
