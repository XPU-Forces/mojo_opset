...
from mojo_opset.kernels._triton_utils import input_guard
from mojo_opset.kernels._triton_utils import prepare_chunk_indices

from .silu import silu_bwd
...
def causal_conv1d_fwd_kernel(
    ...
    for task_id in range(pid, total_tasks, num_programs):
        ...
        out_t = i_t * BT + tl.arange(0, BT)
        out_offsets = (bos + out_t[:, None]) * D + o_d[None, :]
        tl.store(y + out_offsets, b_y.to(y.dtype.element_ty), mask=(out_t[:, None] < T_len) & m_d[None, :])
...
def causal_conv1d_bwd_kernel(
    ...
    for task_id in range(pid, total_tasks, num_programs):
        ...
        if not USE_INITIAL_STATE:
            ...
            b_dy = tl.load(p_dy, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
            ...
            for i_w in tl.static_range(0, W):
                ...
                if HAS_WEIGHT:
                    ...
                    tl.store(dw + i_tg * D * W + (W - i_w - 1) * D + o_d, b_dw.to(dw.dtype.element_ty), mask=m_d)
                if HAS_BIAS and i_w == 0:
                    b_db += tl.sum(b_dy, 0)
                b_dx += b_wdy
        else:
            ...
            for i_w in tl.static_range(0, W):
                p_dy = tl.make_block_ptr(dy + bos * D, (T_len, D), (D, 1), (i_t * BT + i_w, i_d * BD), (BT, BD), (1, 0))
                b_dy_shift = tl.load(p_dy, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
                if ACTIVATION == "swish" or ACTIVATION == "silu":
                    p_y = tl.make_block_ptr(
                        y + bos * D, (T_len, D), (D, 1), (i_t * BT + i_w, i_d * BD), (BT, BD), (1, 0)
                    )
                ...
                if HAS_WEIGHT:
                    ...
                    if USE_INITIAL_STATE:
                        mask_head_rows = (o_t < i_w) & (o_t < T_len)
            ...
            if USE_INITIAL_STATE:
                ...
                b_dy0 = tl.load(p_dy0, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
                if ACTIVATION == "swish" or ACTIVATION == "silu":
...
def causal_conv1d_fwd_impl(
    ...
    y = torch.empty_like(x, dtype=torch.float32 if output_float32 else x.dtype)
...
def causal_conv1d_bwd_impl(
    ...
    causal_conv1d_bwd_kernel[grid](
        ...
        cu_seqlens=cu_seqlens,
        chunk_indices=chunk_indices,
        B=cu_seqlens.numel() - 1 if cu_seqlens is not None else B,
        T=T,
        D=D,
        W=W,
        BT=BT,
...
@torch.library.custom_op("mojo_npu_triton_a2::causal_conv1d_forward", mutates_args=())
def causal_conv1d_forward(
    x: torch.Tensor,
    weight: torch.Tensor,
    ...
    output, _ = causal_conv1d_fwd_impl(
        x,
...
@causal_conv1d_forward.register_fake
def _causal_conv1d_forward_fake(x, weight, bias, residual, initial_state, activation, cu_seqlens, output_float32=False):
    return torch.empty_like(
        x, dtype=torch.float32 if output_float32 else x.dtype, memory_format=torch.contiguous_format
    )
...
def causal_conv1d_update_state_infer_fwd(x, conv_state, weight, bias, activation):
    ...
    # Reuse the forward and rolling-state leaves, keeping both visible to capture.
    compute_x = x.to(weight.dtype).transpose(1, 2).contiguous()
    compute_state = conv_state.to(weight.dtype)
    history = compute_state[..., compute_state.shape[-1] - (weight.shape[1] - 1):]
    output = causal_conv1d_forward(
        compute_x, weight, bias, None, _padded_state(history).contiguous(), activation, None,
    )
    if conv_state.shape[-1]:
        next_state = causal_conv1d_state(compute_x, compute_state, conv_state.shape[-1], None)
        conv_state.copy_(next_state)
    return output.transpose(1, 2).to(x.dtype).contiguous()


def _padded_state(state):
    return None if state is None else torch.nn.functional.pad(state, (1, 0))


def causal_conv1d_fwd(x, weight, bias, residual, initial_state, output_final_state, activation, cu_seqlens):
    output = causal_conv1d_forward(x, weight, bias, residual, _padded_state(initial_state), activation, cu_seqlens)
    final_state = None
    if output_final_state:
        final_state = causal_conv1d_state(x, initial_state, weight.shape[1] - 1, cu_seqlens)
    return output, final_state


@torch.library.custom_op("mojo_npu_triton_a2::causal_conv1d_state", mutates_args=())
def causal_conv1d_state(
    x: torch.Tensor,
    initial_state: Optional[torch.Tensor],
    state_length: int,
    cu_seqlens: Optional[torch.Tensor],
) -> torch.Tensor:
    # Use the existing independent state-update kernel. Its internal state width
    # includes the unused oldest element; public states contain W - 1 elements.
    padded = _padded_state(initial_state)
    full_state = causal_conv1d_update_states(x, state_length + 1, padded, cu_seqlens)
    return full_state[..., 1:].contiguous()


@causal_conv1d_state.register_fake
def _causal_conv1d_state_fake(x, initial_state, state_length, cu_seqlens):
    batches = x.shape[0] if cu_seqlens is None else cu_seqlens.shape[0] - 1
    return x.new_empty((batches, x.shape[-1], state_length))


@torch.library.custom_op("mojo_npu_triton_a2::causal_conv1d_backward", mutates_args=())
def causal_conv1d_backward(
    dy: torch.Tensor,
    x: torch.Tensor,
    weight: torch.Tensor,
    bias: Optional[torch.Tensor],
    ...
    dx, dw, db, _, dh = causal_conv1d_bwd_impl(
        x.contiguous(),
        dy.contiguous(),
        None,
        weight.transpose(0, 1).contiguous(),
    ...
    return dx, dw, x.new_empty((0,)) if db is None else db, x.new_empty((0,)) if dh is None else dh


@causal_conv1d_backward.register_fake
def _causal_conv1d_backward_fake(dy, x, weight, bias, initial_state, cu_seqlens):
    return (
        torch.empty_like(x, memory_format=torch.contiguous_format),
        torch.empty_like(weight, memory_format=torch.contiguous_format),
        x.new_empty((0,)) if bias is None else torch.empty_like(bias),
        x.new_empty((0,)) if initial_state is None else torch.empty_like(initial_state),
    )


def causal_conv1d_bwd(dy, dstate, x, weight, bias, residual, initial_state, output_final_state, activation, cu_seqlens):
    padded = _padded_state(initial_state)
    conv_dy = dy
    if activation is not None:
        # Compose existing leaves so both recomputation and the activation
        # derivative remain visible in the backward graph.
        # Keep the derivative intermediate in FP32; BF16 rounding before the
        # weight-gradient reduction would add an avoidable quantization step.
        preactivation = causal_conv1d_forward(x, weight, bias, None, padded, None, cu_seqlens, True)
        conv_dy = silu_bwd(dy, preactivation)
    dx, dw, db, dh = causal_conv1d_backward(conv_dy, x, weight, bias, padded, cu_seqlens)
    dh = None if initial_state is None else dh[..., 1:]
    if output_final_state and dstate is not None:
        sx, sh = causal_conv1d_state_backward(dstate, x, initial_state, cu_seqlens)
        dx = dx + sx
        if dh is not None:
            dh = dh + sh
    return dx, dw, None if bias is None else db, None if residual is None else dy, dh


@torch.library.custom_op("mojo_npu_triton_a2::causal_conv1d_state_backward", mutates_args=())
def causal_conv1d_state_backward(
    dstate: torch.Tensor,
    x: torch.Tensor,
    initial_state: Optional[torch.Tensor],
    cu_seqlens: Optional[torch.Tensor],
) -> tuple[torch.Tensor, torch.Tensor]:
    dx = torch.zeros_like(x, memory_format=torch.contiguous_format)
    dh = (
        x.new_empty((0,))
        if initial_state is None
        else torch.zeros_like(initial_state, memory_format=torch.contiguous_format)
    )
    batches = x.shape[0] if cu_seqlens is None else cu_seqlens.shape[0] - 1
    width = dstate.shape[-1]
    grid = (triton.cdiv(x.shape[-1], 64), batches)
    _state_backward_kernel[grid](
        dstate.contiguous(),
        dx,
        dh,
        cu_seqlens,
        ...
        BD=64,
        BW=triton.next_power_of_2(max(width, 1)),
    )
    return dx, dh


@causal_conv1d_state_backward.register_fake
def _causal_conv1d_state_backward_fake(dstate, x, initial_state, cu_seqlens):
    return (
        torch.empty_like(x, memory_format=torch.contiguous_format),
        x.new_empty((0,))
        if initial_state is None
        else torch.empty_like(initial_state, memory_format=torch.contiguous_format),
    )
...
def _state_backward_kernel(
    ...
    length = end - start
    pos = length - W + w
    g = tl.load(ds + batch * D * W + d[:, None] * W + w[None, :], mask=(d[:, None] < D) & (w[None, :] < W), other=0)
    tl.store(
        dx + (start + pos[None, :]) * D + d[:, None], g, mask=(d[:, None] < D) & (w[None, :] < W) & (pos[None, :] >= 0)
    )
    if HAS_INITIAL:
        tl.store(
            dh + batch * D * W + d[:, None] * W + (pos + W)[None, :],
            g,
            mask=(d[:, None] < D) & (w[None, :] < W) & (pos[None, :] < 0),
        )
