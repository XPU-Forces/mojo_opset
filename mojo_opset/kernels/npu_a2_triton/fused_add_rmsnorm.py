...
from mojo_opset.kernels._npu_triton_utils import VEC_ALIGN_BYTES, get_num_cores
from mojo_opset.kernels._triton_utils import align, ceil_div
...
@torch.library.custom_op("mojo_npu_triton_a2::residual_add_rms_norm_infer", mutates_args=())
def residual_add_rms_norm_infer_fwd(
    x: torch.Tensor, residual: torch.Tensor, weight: torch.Tensor, eps: float,
) -> tuple[torch.Tensor, torch.Tensor]:
    return fused_add_rmsnorm_infer_impl(x.contiguous(), residual.contiguous(), weight.contiguous(), "pre", eps)
