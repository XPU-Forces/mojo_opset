...
@torch.library.custom_op("mojo_npu_triton_a2::store_paged_kv_cache", mutates_args=("key_cache", "value_cache"))
def _store_paged_kv_leaf(key_states: torch.Tensor, value_states: torch.Tensor, key_cache: torch.Tensor,
                         value_cache: torch.Tensor, chunk_metadata: torch.Tensor) -> None:
    store_paged_kv_impl(key_states, value_states, key_cache, value_cache, chunk_metadata)


@_store_paged_kv_leaf.register_fake
def _store_paged_kv_fake(key_states, value_states, key_cache, value_cache, chunk_metadata):
    return None
...
def store_paged_kv_cache_fwd(key_states, value_states, key_cache, value_cache, block_table,
    ...
    if chunk_metadata is None:
        chunk_metadata = build_paged_kv_chunk_metadata(block_table, cu_q_lens, context_kv_lens,
                                                      key_cache.shape[2])
    _store_paged_kv_leaf(key_states, value_states, key_cache, value_cache, chunk_metadata)
    return key_cache, value_cache
