...
from .rmsnorm import rms_norm_infer_fwd

# The grouped kernel retains more weight/select buffers than standalone RMS.
# Wide fused tiles exceed A5 UB (D=4096 at the original tile; G=4,D=10240 even
# at M=1). A dynamic-weight-loading alternative compiled but produced incorrect
# affine results on 950. Keep the proven fused range; wider groups use the
# existing per-group RMS kernels from this same provider, not another backend.
GROUP_FUSED_MAX_COLS = 2048
...
def group_rmsnorm_impl(
    ...
    BLOCK_SIZE_N = align(input_groups[0], N, VEC_ALIGN_BYTES) if N <= GROUP_FUSED_MAX_COLS else GROUP_FUSED_MAX_COLS
    BLOCK_SIZE_M = rms_norm_fwd_heuristics({"n_cols": N})
    # Grouped RMS additionally materializes the selected per-row weight. The
    # standalone RMS tile (e.g. M=6 at D=4096) exceeds A5 UB in this kernel.
    # Keep the original small-head tiles, and reduce only wide-row tiling.
    if N >= 2048:
        BLOCK_SIZE_M = 1

    use_fused = N <= GROUP_FUSED_MAX_COLS and BLOCK_SIZE_N >= N and 2 <= G <= 4
...
@torch.library.custom_op("mojo_npu_triton_a5::group_rms_norm_infer", mutates_args=())
def group_rms_norm_fused(
    input_groups: list[torch.Tensor], weight: Optional[torch.Tensor], eps: float,
) -> list[torch.Tensor]:
    # Preserve adjacent packed views: the original helper can reuse their
    # allocation. Pointer inspection stays inside this opaque single leaf.
    first = input_groups[0]
    weight = first.new_ones((len(input_groups), first.shape[-1])) if weight is None else weight.contiguous()
    outputs = group_rmsnorm_impl(input_groups, weight, eps, output_like_input_stride=False)
    return [y.clone(memory_format=torch.contiguous_format) for y in outputs]


@group_rms_norm_fused.register_fake
def _group_rms_norm_fused_fake(input_groups, weight, eps):
    return [torch.empty_like(x, memory_format=torch.contiguous_format) for x in input_groups]


def group_rms_norm_infer_fwd(input_groups, weight, eps):
    if any(x.ndim != 3 for x in input_groups):
        raise NotImplementedError("Triton group RMSNorm requires 3D groups")
    # Match the original raw-kernel specialization, independent of storage:
    # small groups are fused (concatenated if necessary), otherwise one launch
    # per group. Ordinary Python keeps those individual leaves in the graph.
    if 2 <= len(input_groups) <= 4 and input_groups[0].shape[-1] <= GROUP_FUSED_MAX_COLS:
        if any(x.shape[0] != input_groups[0].shape[0] for x in input_groups):
            raise NotImplementedError("Fused Triton group RMSNorm requires groups sharing the token count")
        return group_rms_norm_fused(input_groups, weight, eps)
    return [rms_norm_infer_fwd(x, x.new_ones(x.shape[-1]) if weight is None else weight[i], eps)
