...
@torch.library.custom_op("mojo_npu_triton_a5::quant_gemm_fwd", mutates_args=())
...
def _quant_gemm_fake(input, weight, input_scale, weight_scale, output_dtype, trans_weight):
    n = weight.shape[0] if trans_weight else weight.shape[1]
    m = input.shape[0]
    # The real leaf returns a view of its padded GEMM output, not a compact copy.
    padded = input.new_empty((_pad_to(max(m, 1), _PAD_M), _pad_to(n, _PAD_N)), dtype=output_dtype)
    return padded[:m, :n]
