...
@torch.library.custom_op("mojo_npu_triton_a5::store_lowrank_fwd", mutates_args=("label_cache",))
def store_lowrank_fwd(label_cache: torch.Tensor, key_lr: torch.Tensor, block_idxs: torch.Tensor,
                      token_idxs: torch.Tensor, token_num: int) -> None:
    store_label_cache_infer_impl(label_cache, key_lr, block_idxs, token_idxs, token_num)


@store_lowrank_fwd.register_fake
def _store_lowrank_fake(label_cache, key_lr, block_idxs, token_idxs, token_num):
    return None
