...
@torch.library.custom_op("mojo_npu_triton_a5::static_quant_fwd", mutates_args=())
def static_quant_fwd(input: torch.Tensor, scale: torch.Tensor,
                     quant_dtype: torch.dtype) -> torch.Tensor:
    if quant_dtype != torch.int8:
        raise NotImplementedError("NPU Triton static_quant supports int8 only")
    if input.numel() == 0:
        return torch.empty_like(input, dtype=torch.int8)
    return static_quant_impl(input.contiguous(), scale.contiguous())


@static_quant_fwd.register_fake
def _static_quant_fake(input, scale, quant_dtype):
    if quant_dtype != torch.int8:
        raise NotImplementedError("NPU Triton static_quant supports int8 only")
    return torch.empty_like(input, dtype=quant_dtype, memory_format=torch.contiguous_format)


@torch.library.custom_op("mojo_npu_triton_a5::dequant_fwd", mutates_args=())
def dequant_fwd(input: torch.Tensor, scale: torch.Tensor,
                output_dtype: torch.dtype) -> torch.Tensor:
    if scale.ndim == 0 or input.shape[-scale.ndim:] != scale.shape:
        raise NotImplementedError("NPU Triton dequant requires trailing-dimension scales")
    if input.numel() == 0:
        return torch.empty_like(input, dtype=output_dtype)
    return dequant_impl(input.contiguous(), scale.contiguous(), output_dtype)


@dequant_fwd.register_fake
def _dequant_fake(input, scale, output_dtype):
    if scale.ndim == 0 or input.shape[-scale.ndim:] != scale.shape:
        raise NotImplementedError("NPU Triton dequant requires trailing-dimension scales")
    return torch.empty_like(input, dtype=output_dtype, memory_format=torch.contiguous_format)


@torch.library.custom_op("mojo_npu_triton_a5::dynamic_quant_fwd", mutates_args=())
def dynamic_quant_fwd(input: torch.Tensor, inv_smooth_scale: Optional[torch.Tensor] = None
    ...
    flat = input.contiguous().reshape(-1, input.shape[-1])
    smooth = None if inv_smooth_scale is None else inv_smooth_scale.contiguous()
    output, scale = dynamic_quant_impl(flat, smooth)
    return output.reshape(input.shape), scale.reshape(*input.shape[:-1], 1)
...
def moe_dynamic_quant_fwd(input, token_count, inv_smooth_scale):
    # Original TTXMoEDynamicQuant: grouped smoothing is performed in FP32,
    # then the ordinary dynamic-quant Triton leaf quantizes those values.
    flat = input.reshape(-1, input.shape[-1])
    smooth = inv_smooth_scale.float().repeat_interleave(token_count, dim=0,
                                                       output_size=flat.shape[0])
    output, scale = dynamic_quant_fwd(flat.float() * smooth, None)
    return output.reshape(input.shape), scale.reshape(*input.shape[:-1], 1)
