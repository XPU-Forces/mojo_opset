...
# Source: origin/master mojo_opset/backends/ttx/functions/loss_function.py.
# Raw numerical kernels above are preserved; these adapters only expose schemas.
@torch.library.custom_op(
    "mojo_npu_triton_a5::linear_cross_entropy_reduce", mutates_args=(),
    schema="(Tensor inputs, Tensor weight, Tensor target, Tensor? bias, Tensor? ce_weight, int ignore_index, "
           "float lse_square_scale, float label_smoothing, str reduction, float? softcap, "
           "bool return_z_loss, ScalarType? accum_dtype, bool weight_requires_grad) -> (Tensor, Tensor?, Tensor, Tensor?, Tensor?)",
)
def linear_cross_entropy_reduce(
    inputs: torch.Tensor, weight: torch.Tensor, target: torch.Tensor,
    bias: Optional[torch.Tensor], ce_weight: Optional[torch.Tensor], ignore_index: int,
    lse_square_scale: float, label_smoothing: float, reduction: str, softcap: Optional[float],
    return_z_loss: bool, accum_dtype: Optional[torch.dtype], weight_requires_grad: bool,
    ...
    return fused_linear_cross_entropy_fwd_impl(
        inputs, weight.detach().requires_grad_(weight_requires_grad), target, ce_weight, bias,
        ignore_index, lse_square_scale, label_smoothing, reduction, softcap, return_z_loss, accum_dtype,
    )


@linear_cross_entropy_reduce.register_fake
def _linear_cross_entropy_reduce_fake(inputs, weight, target, bias, ce_weight, ignore_index,
                                     lse_square_scale, label_smoothing, reduction, softcap,
                                     return_z_loss, accum_dtype, weight_requires_grad):
    return (inputs.new_empty((), dtype=torch.float32),
            inputs.new_empty(()) if return_z_loss else None,
            torch.empty_like(inputs),
            torch.empty_like(weight) if weight_requires_grad else None,
            torch.empty_like(bias) if bias is not None else None)


@torch.library.custom_op(
    "mojo_npu_triton_a5::linear_cross_entropy_none", mutates_args=(),
    schema="(Tensor inputs, Tensor weight, Tensor target, Tensor? bias, Tensor? ce_weight, int ignore_index, "
           "float lse_square_scale, float label_smoothing, float? softcap, bool return_z_loss) -> (Tensor, Tensor?)",
)
def linear_cross_entropy_none(
    inputs: torch.Tensor, weight: torch.Tensor, target: torch.Tensor,
    bias: Optional[torch.Tensor], ce_weight: Optional[torch.Tensor], ignore_index: int,
    lse_square_scale: float, label_smoothing: float, softcap: Optional[float], return_z_loss: bool,
) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
    return fused_linear_cross_entropy_1d_fwd_impl(
        inputs, weight, target, ce_weight, bias, ignore_index,
        lse_square_scale, label_smoothing, softcap, return_z_loss,
    )


@linear_cross_entropy_none.register_fake
def _linear_cross_entropy_none_fake(inputs, weight, target, bias, ce_weight, ignore_index,
                                   lse_square_scale, label_smoothing, softcap, return_z_loss):
    return (inputs.new_empty((inputs.shape[0],), dtype=torch.float32),
            inputs.new_empty((inputs.shape[0],)) if return_z_loss else None)


@torch.library.custom_op(
    "mojo_npu_triton_a5::linear_cross_entropy_none_backward", mutates_args=(),
    schema="(Tensor grad_loss, Tensor inputs, Tensor weight, Tensor target, Tensor? bias, Tensor? ce_weight, "
           "int ignore_index, float lse_square_scale, float label_smoothing, float? softcap, "
           "ScalarType? accum_dtype) -> (Tensor, Tensor, Tensor?)",
)
def linear_cross_entropy_none_backward(
    grad_loss: torch.Tensor, inputs: torch.Tensor, weight: torch.Tensor, target: torch.Tensor,
    bias: Optional[torch.Tensor], ce_weight: Optional[torch.Tensor], ignore_index: int,
    lse_square_scale: float, label_smoothing: float, softcap: Optional[float], accum_dtype: Optional[torch.dtype],
) -> tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
    return fused_linear_cross_entropy_1d_bwd_impl(
        grad_loss, inputs, weight, target, ce_weight, bias, ignore_index,
        lse_square_scale, label_smoothing, softcap, accum_dtype,
    )


@linear_cross_entropy_none_backward.register_fake
def _linear_cross_entropy_none_backward_fake(grad_loss, inputs, weight, target, bias, ce_weight,
                                            ignore_index, lse_square_scale, label_smoothing, softcap, accum_dtype):
    return torch.empty_like(inputs), torch.empty_like(weight), torch.empty_like(bias) if bias is not None else None
...
def linear_cross_entropy_fwd(inputs, weight, target, bias, ce_weight, ignore_index,
                             ...
                             return_z_loss, accum_dtype, weight_requires_grad):
    if reduction == "none":
        loss, zloss = linear_cross_entropy_none(
            inputs, weight, target, bias, ce_weight, ignore_index,
            lse_square_scale, label_smoothing, softcap, return_z_loss,
        )
        return loss, zloss, None, None, None
    return linear_cross_entropy_reduce(
        inputs, weight, target, bias, ce_weight, ignore_index, lse_square_scale,
        label_smoothing, reduction, softcap, return_z_loss, accum_dtype, weight_requires_grad,
    )


def linear_cross_entropy_bwd(grad_loss, grad_zloss, inputs, weight, target, bias, ce_weight, ignore_index,
                             lse_square_scale, label_smoothing, reduction, softcap,
                             return_z_loss, accum_dtype, dx, dw, db):
    # Original TTX treats z-loss as logging only.
    if reduction == "none":
        return linear_cross_entropy_none_backward(
            grad_loss.contiguous(), inputs, weight, target, bias, ce_weight, ignore_index,
            lse_square_scale, label_smoothing, softcap, accum_dtype,
        )
    # Do not mutate saved gradients: repeated backward must not rescale them twice.
    return tuple(None if g is None else g * grad_loss for g in (dx, dw, db))
