...
def top_k_sampling_impl(
    ...
    probs_data = top_k_probabilities(sorted_logits, sorted_topk_indices)
...
@torch.library.custom_op("mojo_npu_triton_a5::top_k_probabilities", mutates_args=())
def top_k_probabilities(sorted_logits: torch.Tensor, sorted_indices: torch.Tensor) -> torch.Tensor:
    rows, top_k = sorted_logits.shape
    # Original A5 computes an unused sample alongside probabilities. Its empty
    # threshold buffer does not consume RNG; multinomial remains in Python.
    rand_data = torch.empty(rows, dtype=torch.float32, device=sorted_logits.device)
    output_data = torch.empty((rows, 2), dtype=torch.float32, device=sorted_logits.device)
    probabilities = torch.empty((rows, top_k), dtype=torch.float32, device=sorted_logits.device)
    _top_k_sample_kernel[(rows,)](
        sorted_logits, sorted_indices, rand_data, output_data, probabilities,
        sorted_logits.stride(0), sorted_logits.stride(1),
        sorted_indices.stride(0), sorted_indices.stride(1), rand_data.stride(0),
        output_data.stride(0), output_data.stride(1),
        probabilities.stride(0), probabilities.stride(1), TOP_K=top_k,
    )
    return probabilities


@top_k_probabilities.register_fake
def _top_k_probabilities_fake(sorted_logits, sorted_indices):
    return torch.empty(sorted_logits.shape, dtype=torch.float32, device=sorted_logits.device)

@torch.library.custom_op("mojo_npu_triton_a5::top_p_sample", mutates_args=())
...
@torch.library.custom_op("mojo_npu_triton_a5::top_p_scores", mutates_args=())
...
@torch.library.custom_op("mojo_npu_triton_a5::top_p_filter_probs", mutates_args=())
...
@torch.library.custom_op("mojo_npu_triton_a5::reject_sample", mutates_args=())
...
@torch.library.custom_op("mojo_npu_triton_a5::join_prob_reject_sample", mutates_args=())
...
@torch.library.custom_op("mojo_npu_triton_a5::penalties_temperature", mutates_args=("logits",))
