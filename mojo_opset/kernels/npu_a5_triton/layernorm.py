...
from mojo_opset.kernels._triton_utils import align, ceil_div, torch_to_triton_dtype
...
@torch.library.custom_op("mojo_npu_triton_a5::layer_norm_infer", mutates_args=())
def layer_norm_infer_fwd(
    x: torch.Tensor, weight: Optional[torch.Tensor], bias: Optional[torch.Tensor], eps: float,
) -> torch.Tensor:
    weight = x.new_ones(x.shape[-1]) if weight is None else weight.contiguous()
    bias = x.new_zeros(x.shape[-1]) if bias is None else bias.contiguous()
    return layernorm_infer_impl(x.contiguous(), weight, bias, eps)
