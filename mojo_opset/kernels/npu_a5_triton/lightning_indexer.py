...
@torch.library.custom_op("mojo_npu_triton_a5::lightning_indexer_fwd", mutates_args=())
def lightning_indexer_fwd(query: torch.Tensor, query_scale: torch.Tensor, key: torch.Tensor,
                          key_scale: torch.Tensor) -> torch.Tensor:
    return lightning_indexer_impl(query, query_scale, key, key_scale)


@lightning_indexer_fwd.register_fake
def _lightning_indexer_fake(query, query_scale, key, key_scale):
    return query.new_empty((query.shape[0], query.shape[1], key.shape[1]), dtype=torch.float32)
