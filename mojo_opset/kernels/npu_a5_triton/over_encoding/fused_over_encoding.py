...
@torch.library.custom_op("mojo_npu_triton_a5::over_encoding_decode_fwd", mutates_args=())
def over_encoding_decode_fwd(input_ids: torch.Tensor, history: torch.Tensor, sizes: torch.Tensor,
                              offsets: torch.Tensor, grams: torch.Tensor, qweight: torch.Tensor,
                              scale: torch.Tensor, mean: torch.Tensor, ori_vocab_size: int, group_size: int,
                              codebook: Optional[torch.Tensor], vocab_start_id: int, output_dtype: torch.dtype) -> torch.Tensor:
    return over_encoding_decode_impl(input_ids, history, sizes, offsets, grams, qweight, scale, mean,
                                      ori_vocab_size=ori_vocab_size, group_size=group_size, codebook=codebook,
                                      mega_vocab_start_id=vocab_start_id, output_dtype=output_dtype)


@over_encoding_decode_fwd.register_fake
def _over_encoding_decode_fake(input_ids, history, sizes, offsets, grams, qweight, scale, mean, ori_vocab_size,
                                group_size, codebook, vocab_start_id, output_dtype):
    return input_ids.new_empty((*input_ids.shape, sizes.shape[0], scale.shape[1] * group_size), dtype=output_dtype)
