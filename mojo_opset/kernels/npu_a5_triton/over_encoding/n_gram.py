...
def n_gram_prefill_impl(
    ...
    input_offsets = torch.cumsum(
        torch.cat((q_lens.new_zeros(1), q_lens), dim=0),
        dim=0,
    )
    output = _n_gram_prefill_leaf(input_ids, input_offsets, oe_history_inputs,
                                 oe_vocab_sizes, oe_vocab_offsets, n_grams, vocab_size)
    return output.transpose(0, 1).contiguous()


def _n_gram_prefill_launch(
    input_ids, input_offsets, oe_history_inputs, oe_vocab_sizes,
    oe_vocab_offsets, n_grams, vocab_size,
):
    num_sequences = input_offsets.shape[0] - 1
    output = torch.empty(
        oe_vocab_sizes.size(0), input_ids.size(0), dtype=input_ids.dtype, device=input_ids.device
    )
    ...
    n_gram_prefill_kerenl[grid](
        ...
        BLOCK_IDX_SIZE=math.ceil(n_grams.size(0) * num_sequences / num_programs),
...
@torch.library.custom_op("mojo_npu_triton_a5::n_gram_prefill", mutates_args=())
def _n_gram_prefill_leaf(input_ids: torch.Tensor, input_offsets: torch.Tensor, history: torch.Tensor,
                        sizes: torch.Tensor, offsets: torch.Tensor, grams: torch.Tensor, vocab_size: int) -> torch.Tensor:
    return _n_gram_prefill_launch(input_ids, input_offsets, history, sizes, offsets, grams, vocab_size)


@_n_gram_prefill_leaf.register_fake
def _n_gram_prefill_fake(input_ids, input_offsets, history, sizes, offsets, grams, vocab_size):
    return input_ids.new_empty((sizes.shape[0], input_ids.shape[0]))


n_gram_prefill_fwd = n_gram_prefill_impl


@torch.library.custom_op("mojo_npu_triton_a5::n_gram_decode_fwd", mutates_args=())
def n_gram_decode_fwd(input_ids: torch.Tensor, history: torch.Tensor, sizes: torch.Tensor,
                      offsets: torch.Tensor, grams: torch.Tensor, vocab_size: int) -> torch.Tensor:
    return n_gram_decode_impl(input_ids, history, sizes, offsets, grams, vocab_size)


@n_gram_decode_fwd.register_fake
def _n_gram_decode_fake(input_ids, history, sizes, offsets, grams, vocab_size):
    return input_ids.new_empty((*input_ids.shape, sizes.shape[0]))
