...
@torch.library.custom_op("mojo_npu_triton_a5::embedding_nf4_dequant_fwd", mutates_args=())
def embedding_nf4_dequant_fwd(input_ids: torch.Tensor, qweight: torch.Tensor, scale: torch.Tensor, mean: torch.Tensor,
                               group_size: int, codebook: Optional[torch.Tensor], vocab_start_id: int,
                               output_dtype: torch.dtype) -> torch.Tensor:
    return embedding_nf4_dequant_impl(input_ids, qweight, scale, mean, group_size=group_size, codebook=codebook,
                                      vocab_start_id=vocab_start_id, output_dtype=output_dtype)


@embedding_nf4_dequant_fwd.register_fake
def _embedding_nf4_dequant_fake(input_ids, qweight, scale, mean, group_size, codebook, vocab_start_id, output_dtype):
    return input_ids.new_empty((*input_ids.shape, scale.shape[1] * group_size), dtype=output_dtype)
