...
def _run_rope_kernel(
    ...
    """
    q, q_bs, q_ss, q_need_tb, batch_size, seq_len, n_qh, head_dim = _normalize_for_rope(q, head_first)
    k, k_bs, k_ss, k_need_tb, _, _, n_kh, _ = _normalize_for_rope(k, head_first)
    ...
    _rope_kernel[grid](
        ...
        ALIGNED=True,
        INVERSE=inverse,
        CACHE_HALVES_REPEATED=cache_halves_repeated,
...
def rope_train_fwd_impl(
    ...
    Computation uses FP32 intermediates for Seed-compatible training semantics.
    """
    head_first = rope_head_first(cos.dim(), unsqueeze_dim)
    return _run_rope_kernel(
        q, k, cos, sin, head_first, inverse=False,
        cache_halves_repeated=cache_halves_repeated,
        cast_cache_to_fp32=True,
...
def rope_train_bwd_impl(
    ...
    head_first = rope_head_first(cos.dim(), unsqueeze_dim)
    return _run_rope_kernel(
        dq, dk, cos, sin, head_first, inverse=True,
        cache_halves_repeated=cache_halves_repeated,
        cast_cache_to_fp32=True,
        contiguous_output=True,
    )
...
def rope_fwd_impl(
    ...
    return _run_rope_kernel(
        q, k, cos, sin, head_first, inverse=False,
        cache_halves_repeated=True,
        cast_cache_to_fp32=False,
...
@torch.library.custom_op("mojo_npu_triton_a5::rope_fwd", mutates_args=())
def rope_fwd(
...
@torch.library.custom_op("mojo_npu_triton_a5::rope_bwd", mutates_args=())
def rope_bwd(
...
@torch.library.custom_op("mojo_npu_triton_a5::rope_infer_fwd", mutates_args=())
...
@torch.library.custom_op("mojo_npu_triton_a5::rope_cos_sin_fwd", mutates_args=())
