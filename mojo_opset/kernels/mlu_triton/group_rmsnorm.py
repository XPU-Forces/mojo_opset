...
from mojo_opset.kernels.mlu_triton.utils import get_mlu_total_cores
...
@torch.library.custom_op("mojo_mlu_triton::group_rms_norm_single_infer", mutates_args=())
def group_rms_norm_single_infer(
    x: torch.Tensor, weight: Optional[torch.Tensor], eps: float,
) -> torch.Tensor:
    return rmsnorm_fwd_triton(x.contiguous(), None if weight is None else weight.contiguous(),
               eps, output_like_input_stride=False)


@group_rms_norm_single_infer.register_fake
def _group_rms_norm_single_infer_fake(x, weight, eps):
    return torch.empty_like(x, memory_format=torch.contiguous_format)


def group_rms_norm_infer_fwd(input_groups, weight, eps):
    return [group_rms_norm_single_infer(x, None if weight is None else weight[i], eps)
