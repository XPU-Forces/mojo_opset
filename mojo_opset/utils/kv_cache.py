"""Provider-independent planning for in-place paged cache writes."""
...
def assert_paged_kv_layout_contract(block_table: torch.Tensor, cu_q_lens: Optional[torch.Tensor], context_kv_lens: Optional[torch.Tensor]) -> None:
...
def build_paged_kv_chunk_metadata(block_table: torch.Tensor, cu_q_lens: Optional[torch.Tensor], context_kv_lens: torch.Tensor, block_size: int) -> torch.Tensor:
    ...
    if cu_q_lens is None:
        ...
        chunk_metadata = torch.stack((src_token_bases, physical_block, torch.remainder(safe_context, block_size), torch.ones_like(src_token_bases)), dim=-1)
