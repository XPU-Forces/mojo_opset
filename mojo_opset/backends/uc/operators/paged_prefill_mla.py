"""UC backend stub for UCPagedPrefillMLA.

STATUS: DISABLED — kernel 不在当前 uc-kernel wheel 中.

阻塞原因: unic FOR-loop carry shape mismatch (E)
涉及 API : mojo_paged_prefill_mla_bf16
...
class UCPagedPrefillMLA(MojoPagedPrefillMLA):
    ...
    def forward(self, *args, **kwargs):
        raise NotImplementedError(
            "UC backend for UCPagedPrefillMLA is disabled: mojo_paged_prefill_mla_bf16 not available in current uc-kernel wheel. Reason: unic FOR-loop carry shape mismatch (E). See docs/project-ops/uc-kernel-fail-todo-2026-06-08.md. Per project rule '没实现的就直接报错', this wrapper does not silently fall back to torch. Use TTX / torch_npu / torch_native backend instead."
        )
