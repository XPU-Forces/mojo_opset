"""UC backend stub for UCPagedPrefillGQA.

STATUS: DISABLED — kernel 不在当前 uc-kernel wheel 中.

阻塞原因: unic FOR-loop carry shape mismatch + split_dim=0 row_max race
涉及 API : mojo_paged_prefill_gqa_bf16
...
class UCPagedPrefillGQA(MojoPagedPrefillGQA):
    ...
    def forward(self, *args, **kwargs):
        raise NotImplementedError(
            "UC backend for UCPagedPrefillGQA is disabled: mojo_paged_prefill_gqa_bf16 not available in current uc-kernel wheel. Reason: unic FOR-loop carry shape mismatch + split_dim=0 row_max race. See docs/project-ops/uc-kernel-fail-todo-2026-06-08.md. Per project rule '没实现的就直接报错', this wrapper does not silently fall back to torch. Use TTX / torch_npu / torch_native backend instead."
        )
