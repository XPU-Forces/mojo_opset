        ...
        *,
        ...
        if cu_seqlens_kv is None:
            if seqlens_kv is None:
                ...
                if seqlens_kv.dtype != torch.int32:
                    raise ValueError(f"seqlens_kv must be int32, got {seqlens_kv.dtype}.")
                if seqlens_kv.ndim != 1:
                    raise ValueError(f"seqlens_kv must be 1D, got shape {tuple(seqlens_kv.shape)}.")
                if seqlens_kv.shape[0] != q_lens.shape[0]:
                    raise ValueError(
                        "seqlens_kv length must match batch size from cu_seqlens_q, "
                        f"got {seqlens_kv.shape[0]} and {q_lens.shape[0]}."
                    )
                if bool((seqlens_kv < 0).any().item()):
                    raise ValueError("seqlens_kv must be non-negative.")
            ...
            if cu_seqlens_kv.ndim != 1:
                raise ValueError(
                    f"cu_seqlens_kv must be 1D, got shape {tuple(cu_seqlens_kv.shape)}."
                )
            kv_lens = cu_seqlens_kv[1:] - cu_seqlens_kv[:-1]
            if bool((kv_lens < 0).any().item()):
                raise ValueError("cu_seqlens_kv must be monotonically non-decreasing.")
    ...
    def forward(
        ...
        *,
        ...
        if max_context_len is None:
            max_context_len = int(seqlens.max().item()) if seqlens.numel() > 0 else 0
