...
def micro_kernel_fwd(
    block_q, k, v, block_o, block_m, block_l, scale, offset_c, block_mask,
    idx_b, idx_n, idx_h,
    S: tl.constexpr,
    ...
    block_o = tl.exp(block_m - block_m_1)[:, None].to(LOW_TYPE) * block_o + tl.dot(block_s.to(LOW_TYPE), block_v).to(LOW_TYPE)
    return block_o, block_m_1, block_l_1


@triton.jit
def micro_kernel_bwd_q(
    block_q, k, v, block_do, block_d, block_dq, block_lse, 
    scale, offset_c, block_mask,
    idx_b, idx_n, idx_h,
    S: tl.constexpr,
    STRIDE_K_B: tl.constexpr,
    STRIDE_K_N: tl.constexpr,
...
def micro_kernel_bwd_kv(
    q, block_k, block_v, do, d, block_dk, block_dv, lse, 
    scale, offset_r, block_mask,
    idx_b, idx_n, idx_h,
    S: tl.constexpr,
    ...
    ptr_d = (
        ...
        idx_b * STRIDE_D_B + 
        idx_n * STRIDE_D_N + 
        (offset_r + tl.arange(0, BLOCK_R))[:] * STRIDE_D_S
    )
    ptr_lse = (
        lse + 
        idx_b * STRIDE_D_B + 
...
def kernel_sda_fwd_up(
    ...
    for task_id in range(pid, B * N * num_r, tl.num_programs(axis=0)):
        ...
        block_o, block_m, block_l = micro_kernel_fwd(
            block_q, k, v, block_o,
            block_m, block_l, scale, idx_r * BLOCK_R,
            block_mask,
            idx_b, idx_n, idx_h,
            S,
            STRIDE_K_B, STRIDE_K_N, STRIDE_K_S, STRIDE_K_H, 
            STRIDE_V_B, STRIDE_V_N, STRIDE_V_S, STRIDE_V_H, 
            GROUP_SIZE,
            BLOCK_R,
            LOW_TYPE,
            HIGH_TYPE,
        ...
        block_o, block_m, block_l = micro_kernel_fwd(
            block_q, k, v, block_o,
            block_m, block_l, scale, SEQ_LEN + idx_r * BLOCK_R,
            block_mask,
            idx_b, idx_n, idx_h,
            S,
            STRIDE_K_B, STRIDE_K_N, STRIDE_K_S, STRIDE_K_H, 
        ...
        for idx_tile_r in range(idx_r * BLOCK_R // BLOCK_C * BLOCK_C // BLOCK_R, idx_r):
            block_o, block_m, block_l = micro_kernel_fwd(
                block_q, k, v, block_o,
                block_m, block_l, scale, SEQ_LEN + idx_tile_r * BLOCK_R,
                None,
                idx_b, idx_n, idx_h,
                S,
                STRIDE_K_B, STRIDE_K_N, STRIDE_K_S, STRIDE_K_H, 
        ...
        for idx_c in range(idx_r * BLOCK_R // BLOCK_C):
            block_o, block_m, block_l = micro_kernel_fwd(
                block_q, k, v, block_o,
                block_m, block_l, scale, SEQ_LEN + idx_c * BLOCK_C,
                None,
                idx_b, idx_n, idx_h,
                S,
                STRIDE_K_B, STRIDE_K_N, STRIDE_K_S, STRIDE_K_H, 
...
def kernel_sda_fwd_down(
    ...
    for task_id in range(pid, B * N * num_r, tl.num_programs(axis=0)):
        ...
        for idx_tile_r in range(idx_r * BLOCK_R // BLOCK_C * BLOCK_C // BLOCK_R, idx_r):
            block_o, block_m, block_l = micro_kernel_fwd(
                block_q, k, v, block_o,
                block_m, block_l, scale, idx_tile_r * BLOCK_R,
                None,
                idx_b, idx_n, idx_h,
                S,
                STRIDE_K_B, STRIDE_K_N, STRIDE_K_S, STRIDE_K_H, 
        ...
        for idx_c in range(SEQ_LEN // BLOCK_C, idx_r * BLOCK_R // BLOCK_C):
            block_o, block_m, block_l = micro_kernel_fwd(
                block_q, k, v, block_o,
                block_m, block_l, scale, idx_c * BLOCK_C,
                None,
                idx_b, idx_n, idx_h,
                S,
                STRIDE_K_B, STRIDE_K_N, STRIDE_K_S, STRIDE_K_H, 
...
@triton.autotune(configs=[triton.Config({'BLOCK_R': 64}),], key=['N', 'S', 'H'])
@triton.jit
...
def kernel_sda_bwd_q_up(
    ...
    for task_id in range(pid, B * N * num_r, tl.num_programs(axis=0)):
        ...
        for idx_tile_r in range(idx_r * BLOCK_R // BLOCK_C * BLOCK_C // BLOCK_R, idx_r):
            block_dq = micro_kernel_bwd_q(
                block_q, k, v, block_do, block_d, block_dq, block_lse, 
                scale, SEQ_LEN + idx_tile_r * BLOCK_R,
                None,
                idx_b, idx_n, idx_h,
                S,
                STRIDE_K_B, STRIDE_K_N, STRIDE_K_S, STRIDE_K_H, 
...
@triton.autotune(
    configs=[
        triton.Config({"BLOCK_R": 256, "BLOCK_C": 64},
        multibuffer=True,
        unit_flag=True,
        set_workspace_multibuffer=2,
        enable_hivm_auto_cv_balance=True,
    ...
    for task_id in range(pid, B * num_group * num_c, tl.num_programs(axis=0)):
        ...
        for idx_ingroup in range(group_size):
            ...
            block_dk, block_dv = micro_kernel_bwd_kv(
                ...
                idx_b, idx_n, idx_h,
                S,
                STRIDE_Q_B, STRIDE_Q_N, STRIDE_Q_S, STRIDE_Q_H, 
                STRIDE_D_B, STRIDE_D_N, STRIDE_D_S,
                BLOCK_C,
                LOW_TYPE,
                HIGH_TYPE,
            )
    ...
    for task_id in range(pid, B * num_group * num_c, tl.num_programs(axis=0)):
        ...
        for idx_ingroup in range(group_size):
            ...
            for idx_tile_r in range(idx_c + 1, (idx_c * BLOCK_C // BLOCK_R + 1) * BLOCK_R // BLOCK_C):
                block_dk, block_dv = micro_kernel_bwd_kv(
                    q, block_k, block_v, do, d, block_dk, block_dv, lse, 
                    scale, idx_tile_r * BLOCK_C - SEQ_LEN,
                    None,
                    idx_b, idx_n, idx_h,
                    S,
                    STRIDE_Q_B, STRIDE_Q_N, STRIDE_Q_S, STRIDE_Q_H, 
