...
def lightning_indexer_impl(
    ...
    if _lightning_indexer_tle_enabled() and tle is not None:
        try:
            return _lightning_indexer_tle_impl(query, query_scale, key, key_scale)
        except NotImplementedError:
            pass

    return _lightning_indexer_fallback_impl(query, query_scale, key, key_scale)


def _lightning_indexer_fallback_impl(
    query: torch.Tensor,
    query_scale: torch.Tensor,
    key: torch.Tensor,
    key_scale: torch.Tensor,
...
def _lightning_indexer_tle_impl(
    query: torch.Tensor,
    query_scale: torch.Tensor,
    key: torch.Tensor,
    key_scale: torch.Tensor,
    ...
    if tle is None:
        raise NotImplementedError("lightning_indexer TLE path requires triton.experimental.tle")
    ...
    grid = (num_cores,)
    lightning_indexer_tle_kernel[grid](
        query,
        key,
        query_scale,
        key_scale,
...
@libentry()
@triton.jit
def lightning_indexer_tle_kernel(
    query_ptr,
    key_ptr,
    query_scale_ptr,
    key_scale_ptr,
