...
def _compute_rope(
    ...
    x1 = tl.extract_slice(x, [0, 0, 0], [TOKEN_BLOCK_SIZE, head_num, half_head_dim], [1, 1, 1])
    x2 = tl.extract_slice(x, [0, 0, half_head_dim], [TOKEN_BLOCK_SIZE, head_num, half_head_dim], [1, 1, 1])
    ...
    x = tl.insert_slice(x, roped_x1, [0, 0, 0], [TOKEN_BLOCK_SIZE, head_num, half_head_dim], [1, 1, 1])
    x = tl.insert_slice(
        x,
        roped_x2,
        [0, 0, half_head_dim],
        [TOKEN_BLOCK_SIZE, head_num, half_head_dim],
        [1, 1, 1],
    )
...
def _rope_forward_kernel(
    ...
    for block_id in range(pid, total_blocks, grid_size):
        ...
        half_dim_offsets = tl.arange(0, half_hd)
        half_dim_mask = half_dim_offsets < half_hd
        dim_offsets = tl.arange(0, half_hd * 2)
        dim_mask = dim_offsets < half_hd * 2

        cos_block_2d = tl.load(
            cos_token_ptr + half_dim_offsets[None, :], mask=seq_mask[:, None] & half_dim_mask[None, :], other=0
        )
        sin_block_2d = tl.load(
            sin_token_ptr + half_dim_offsets[None, :], mask=seq_mask[:, None] & half_dim_mask[None, :], other=0
        )

        head_q_offsets = tl.arange(0, n_qh)
        head_k_offsets = tl.arange(0, n_kh)

        cos_tile = tl.reshape(cos_block_2d, (TOKEN_BLOCK_SIZE, 1, half_hd), can_reorder=True)
        sin_tile = tl.reshape(sin_block_2d, (TOKEN_BLOCK_SIZE, 1, half_hd), can_reorder=True)
        ...
        q_tile = _compute_rope(q_tile, sin_tile, cos_tile, n_qh, half_hd, TOKEN_BLOCK_SIZE, False)

        tl.store(q_rope_ptr + q_offsets, q_tile, mask=q_mask)
        ...
        k_tile = _compute_rope(k_tile, sin_tile, cos_tile, n_kh, half_hd, TOKEN_BLOCK_SIZE, False)

        tl.store(k_rope_ptr + k_offsets, k_tile, mask=k_mask)
...
def _rope_backward_kernel(
    ...
    for block_id in range(pid, total_blocks, grid_size):
        ...
        dq_tile = _compute_rope(dq_tile, sin_tile, cos_tile, n_qh, half_hd, TOKEN_BLOCK_SIZE, True)

        tl.store(grad_q_ptr + dq_offsets, dq_tile, mask=q_mask)
        ...
        dk_tile = _compute_rope(dk_tile, sin_tile, cos_tile, n_kh, half_hd, TOKEN_BLOCK_SIZE, True)

        tl.store(grad_k_ptr + dk_offsets, dk_tile, mask=k_mask)
...
def rope_fwd_impl(
    ...
    if is_varlen:
        ...
        q = q.transpose(1, 2).contiguous() if head_first else q.contiguous()
        k = k.transpose(1, 2).contiguous() if head_first else k.contiguous()
        batch_size, seq_len, n_q_head, _ = q.shape
        n_kv_head = k.shape[2]
    ...
    return (q_rope, k_rope) if is_varlen or (not head_first) else (q_rope.transpose(1, 2), k_rope.transpose(1, 2))
...
def rope_bwd_impl(
    ...
    if is_varlen:
        ...
        dq = dq.transpose(1, 2).contiguous() if head_first else dq.contiguous()
        dk = dk.transpose(1, 2).contiguous() if head_first else dk.contiguous()
        batch_size, seq_len, n_q_head, _ = dq.shape
        n_kv_head = dk.shape[2]
        dq_batch_stride, dq_seq_stride = dq.stride(0), dq.stride(1)
        dk_batch_stride, dk_seq_stride = dk.stride(0), dk.stride(1)

    grad_q = torch.empty_like(dq)
    grad_k = torch.empty_like(dk)
    ...
    return (
        (grad_q, grad_k)
        if is_varlen or (not head_first)
        else (grad_q.transpose(1, 2).contiguous(), grad_k.transpose(1, 2).contiguous())
    )
