...
def _swa_acc_fwd_dequant_mxn(
    ...
    q_ty = q.type.element_ty
    k = tl.load(k_block_ptr, boundary_check=(0, 1), padding_option="zero").to(q_ty)
    k = k * k_scale[None, :]
    qk = tl.dot(q, tl.trans(k))
    qk = qk * qk_scale
    if mask is not None and mask is not True:
    ...
    qk_safe = tl.where(row_all_masked[:, None], 0.0, qk - m_ij[:, None])
    p = tl.where(row_all_masked[:, None], 0.0, tl.math.exp2(qk_safe))
    p_cast = p.to(q_ty)

    v = tl.load(v_block_ptr, boundary_check=(0, 1), padding_option="zero").to(q_ty)
    v = v * v_scale[None, :]
    l_ij = tl.sum(p, 1)
    alpha = tl.math.exp2(tl.where(row_all_masked, 0.0, m_i - m_ij))
    l_i = l_i * alpha + l_ij
    acc_ptr = acc_ptr * alpha[:, None]
    acc_ptr = tl.dot(p_cast, v, acc_ptr)
    m_i = m_ij
...
@libentry()
@triton.jit
def _swa_acc_fwd_dequant_nomask_mxn(
    acc_ptr,
    l_i,
    m_i,
    q,
...
def _swa_acc_fwd_int8_mxn(
    ...
    q_ty = q_quant.type.element_ty
    k = tl.load(k_block_ptr, boundary_check=(0, 1), padding_option="zero")
    qk = tl.dot(q_quant, tl.trans(k.to(q_ty)))
    qk = qk * q_quant_scale[:, None] * sm_scale
    if mask is not None and mask is not True:
        qk = tl.where(mask, qk, float("-inf"))
    ...
    p_quant = p_rounded.to(tl.int8).to(q_ty)
    ...
    l_i = l_i * alpha + l_ij
    acc_ptr = acc_ptr * alpha[:, None]
    acc_ptr = tl.dot(p_quant, v.to(q_ty), acc_ptr)
    m_i = m_ij
    return acc_ptr, l_i, m_i
...
@libentry()
@triton.jit
def _swa_acc_fwd_int8_nomask_mxn(
    acc_ptr,
    l_i,
    m_i,
    q_quant,
...
@smart_triton_autotune(
    configs=_swa_paged_prefill_autotune_configs(),
    selected_idx=0,
    key=["HEAD_DIM", "GLOBAL_WINDOW_SIZE", "LOCAL_WINDOW_SIZE", "NUM_Q_HEADS", "NUM_KV_HEADS"],
)
@libentry()
@triton.jit
def _swa_paged_prefill_kv_dequant_kernel(
    Q,
    K_cache,
    V_cache,
    Out,
    ...
    for b_id in range(bsz):
        ...
        for q_task_id in range((prev_q_tasks + pid) % n_programs, new_q_tasks, n_programs):
            ...
            if COMPUTE_INT8:
                q_scaled = (q_block.to(tl.float32) * k_scale_vec[None, :]).to(tl.bfloat16).to(tl.float32)
                qi_amax = tl.max(tl.abs(q_scaled), axis=1)
                q_quant_scale_vec = (qi_amax.to(tl.bfloat16) / 127.0).to(tl.bfloat16).to(tl.float32)
                q_quant_scale_vec = tl.where(q_quant_scale_vec < 1.0e-6, 1.0, q_quant_scale_vec)
                q_scaled_norm = (q_scaled / q_quant_scale_vec[:, None]).to(tl.bfloat16).to(tl.float32)
                qi_abs = tl.abs(q_scaled_norm)
                qi_base = tl.floor(qi_abs)
                qi_floor_round = tl.floor(qi_abs + 0.5)
                qi_base_is_even = (qi_base - 2.0 * tl.floor(qi_base * 0.5)) == 0.0
                qi_is_half = (qi_abs - qi_base) == 0.5
                qi_rounded_abs = tl.where(qi_is_half & qi_base_is_even, qi_base, qi_floor_round)
                qi_rounded = tl.where(q_scaled_norm < 0, -qi_rounded_abs, qi_rounded_abs)
                qi_rounded = tl.minimum(tl.maximum(qi_rounded, -128.0), 127.0)
                q_quant = qi_rounded.to(tl.int8).to(q_dtype)
            ...
            for kv_block_id in range(num_global_window_blocks):
                ...
                if IS_CAUSAL:
                    causal_mask = q_abs[:, None] >= kv_abs[None, :]
                    if has_global_window and has_local_window:
                        local_mask = q_abs[:, None] <= (kv_abs + LOCAL_WINDOW_SIZE)[None, :]
                        global_mask = kv_abs[None, :] < GLOBAL_WINDOW_SIZE
                        mask = mask & causal_mask & (global_mask | local_mask)
                    elif has_global_window:
                        mask = mask & causal_mask & (kv_abs[None, :] < GLOBAL_WINDOW_SIZE)
                    elif has_local_window:
                        local_mask = q_abs[:, None] <= (kv_abs + LOCAL_WINDOW_SIZE)[None, :]
                        mask = mask & causal_mask & local_mask
                    else:
                ...
                if COMPUTE_INT8:
                    acc, l_i, m_i = _swa_acc_fwd_int8_mxn(
                        acc, l_i, m_i, q_quant, q_quant_scale_vec,
                        k_block_ptr, v_block_ptr, mask, sm_scale,
                        HEAD_DIM, BLOCK_M, BLOCK_N, BLOCK_D,
                    )
                else:
                    acc, l_i, m_i = _swa_acc_fwd_dequant_mxn(
                        acc, l_i, m_i, q_block,
                        k_block_ptr, v_block_ptr, k_scale_vec, v_scale_vec,
                        mask, sm_scale,
                        HEAD_DIM, BLOCK_M, BLOCK_N, BLOCK_D,
                    )
            ...
            if can_use_nomask_local:
                ...
                if has_local_window:
                    first_local_full_block = tl.maximum(
                        non_global_window_start_block,
                        tl.cdiv(tl.maximum(q_abs_end - LOCAL_WINDOW_SIZE, 0), BLOCK_N),
                    )
                full_local_start_block = first_local_full_block
                full_local_end_block = tl.minimum(num_total_blocks - 1, last_causal_full_block)
                can_use_nomask_local = full_local_start_block <= full_local_end_block
            ...
            if can_use_nomask_local:
                ...
                for kv_block_id in range(full_local_start_block, full_local_end_block + 1):
                    ...
                    if COMPUTE_INT8:
                        acc, l_i, m_i = _swa_acc_fwd_int8_nomask_mxn(
                            acc, l_i, m_i, q_quant, q_quant_scale_vec,
                            k_block_ptr, v_block_ptr, sm_scale,
                            HEAD_DIM, BLOCK_M, BLOCK_N, BLOCK_D,
                        )
                    else:
                        acc, l_i, m_i = _swa_acc_fwd_dequant_nomask_mxn(
                            acc, l_i, m_i, q_block,
                            k_block_ptr, v_block_ptr, k_scale_vec, v_scale_vec,
                            sm_scale,
                            HEAD_DIM, BLOCK_M, BLOCK_N, BLOCK_D,
            ...
            if COMPUTE_INT8:
                ...
                out_block = acc * (1.0 / 127.0) * v_scale_vec[None, :] / l_i_safe[:, None]
            else:
                l_i_safe = tl.where(l_i > 0, l_i, 1.0)
                out_block = tl.where(l_i[:, None] > 0, acc / l_i_safe[:, None], 0.0)
            out_block = tl.where(q_valid[:, None], out_block, 0.0)
            ...
            out_mask = q_valid[:, None] & (offs_d[None, :] < HEAD_DIM)
            tl.store(out_ptrs, out_block.to(Out.type.element_ty), mask=out_mask)
...
def swa_paged_prefill_with_kv_dequant_impl(
    ...
    ``num_stages=1`` is used in all autotune configs to avoid the ILU Triton
    compiler bug where ``tl.dot`` generates invalid bitcasts when the data
    provenance traces back to an int8 load (see commit 1130f65).
    ...
    Args:
        ...
        max_seqlen_kv:   Maximum KV sequence length across the batch.
    """
    total_q_tokens, num_q_heads, head_dim = q.shape
    ...
    # Decouple KV tile width from page_size: large pages (>=128) use BLOCK_N=64 for occupancy.
    if page_size >= 128:
        block_n = 64
    else:
    ...
    if page_size % block_n != 0:
        raise ValueError(
            f"KV block_size ({page_size}) must be divisible by Triton tile size ({block_n}); "
    ...
    _swa_paged_prefill_kv_dequant_kernel[grid](
        q,
        key_cache,
        value_cache,
        out,
        ...
        float(softmax_scale * LOG2E.value),
...
def _paged_decode_kernel(
    ...
    for q_task_id in tl.range(pid, num_tasks, n_progs):
        ...
        # Pipelining disabled: multi-stage prefetch corrupts online-softmax state when
        # PIPELINE_STAGES > 1 (small page_size / BLOCK_SIZE_N tiles on Iluvatar Triton).
        for kv_block_id in tl.range(0, fast_loop_end):
            kv_block_start = kv_block_id * BLOCK_SIZE_N
...
def _paged_decode_quant_kernel(
    ...
    for q_task_id in tl.range(pid, num_tasks, n_progs):
        ...
        # Pipelining is disabled: multi-stage prefetch corrupts online-softmax state for
        # small BLOCK_SIZE_N tiles (e.g. page_size=16) on Iluvatar Triton.
        for kv_block_id in tl.range(0, fast_loop_end):
            kv_block_start = kv_block_id * BLOCK_SIZE_N
            kv_block_end = tl.minimum(kv_block_start + BLOCK_SIZE_N, kv_seq_len)
            kv_block_len = kv_block_end - kv_block_start
