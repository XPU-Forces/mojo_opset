...
ASCEND_ATTN_MASK_BUILDER = None

"""
max_seq_lens can be max_num_batched_tokens, dtypes, device
"""
def init_ascend_attention_mask_builder(max_seq_lens, dtypes, device):
    global ASCEND_ATTN_MASK_BUILDER
    if ASCEND_ATTN_MASK_BUILDER is not None:
        # no need to reinit
        return

    ASCEND_ATTN_MASK_BUILDER = AttentionMaskBuilder(max_seq_lens, dtypes, device)

"""
return the right attention mask base
"""
def make_attention_mask(seq_lens, dtype=None, device=None) -> torch.Tensor:
    global ASCEND_ATTN_MASK_BUILDER
    if ASCEND_ATTN_MASK_BUILDER is None:
        raise RuntimeError("Attn mask builder is none, please run init mask builder first.")

    return ASCEND_ATTN_MASK_BUILDER.get_attn_mask(seq_lens, dtype, device)
