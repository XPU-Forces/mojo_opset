...
def test_moe_ep_replicate_compile_fx_contains_allreduce():
...
def test_moe_ep_sharded_compile_fx_contains_allgather_reducescatter():
    if not hasattr(torch, "compile"):
        pytest.skip("torch.compile is not available.")
    ...
    out_compiled, graph_texts = _compile_capture_fx_and_run(moe, (x,))
    assert torch.allclose(out_compiled, out_eager, rtol=1e-4, atol=1e-4)
    assert _fx_graph_contains(graph_texts, "_c10d_functional.all_gather")
    assert _fx_graph_contains(graph_texts, "_c10d_functional.reduce_scatter")
