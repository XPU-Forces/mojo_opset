...
from mojo_opset.training.operators.attention import MojoDiffusionAttentionFunction
...
def test_diffusion_attention_func(query, key, value, blockwise_diffusion_attn_mask, enable_gqa):
    ctx = MockFunctionCtx()
    o = MojoDiffusionAttentionFunction.forward(ctx, query, key, value, blockwise_diffusion_attn_mask, 1.0)

    ctx_ref = MockFunctionCtx()
    o_ref = MojoDiffusionAttentionFunction._registry.get("torch").forward(
        ctx_ref, query, key, value, blockwise_diffusion_attn_mask, 1.0
    )
