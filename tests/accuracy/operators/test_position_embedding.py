...
def test_pos_emb(bs, seqlen, q_heads, k_heads, head_dim, mode, dtype):
    device = get_platform()
    max_seq_len = 32768
    ...
    if mode == "padding_prefill":
        ...
        q = torch.randn(bs, 1, q_heads, head_dim, device=device, dtype=dtype)
        k = torch.randn(bs, 1, k_heads, head_dim, device=device, dtype=dtype)
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        cu_seqlens = None
    ...
    rope.forward_diff_with(rope_ref, q, k, cos, sin, cu_seqlens, kv_lens, head_first)
