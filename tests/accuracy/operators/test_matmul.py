...
from mojo_opset import MojoGroupQuantMatmulReduceSum, MojoGroupedMatmul
...
def test_group_quant_matmul_reduce_sum(x1, x2, x1_scale, x2_scale, atol, rtol):
    ...
    matmul_ref = MojoGroupQuantMatmulReduceSum._registry.get("torch")()

    matmul.forward_diff_with(matmul_ref, x1, x2, x1_scale, x2_scale, atol=atol, rtol=rtol)
...
def test_grouped_matmul_accuracy(inputs, weights, bias, group_type, dtype, split_item, group_list, output_dtype):
    ...
    grouped_matmul_ref = MojoGroupedMatmul._registry.get("torch")()
    ...
    if group_list is not None:
        ...
        grouped_matmul_op.forward_diff_with(
            grouped_matmul_ref,
            input_tensors,
            weight_tensors,
            bias=bias_tensors,
