...
from mojo_opset import MojoMagicRejectSampling
...
from mojo_opset.backends.reference.operators.sampling import RefRejectMagicRejectSampling
...
@pytest.mark.parametrize(
    ...
    (torch.rand((18, 3, 155136), dtype=torch.float32), torch.randint(0, 155136, (18, 2)), torch.rand((18, 2), dtype=torch.float32), 2, 0.7, 100)],
)
@auto_switch_platform(set_perf=True)
@bypass_not_implemented
def test_reject_sampling(target_logits, draft_tokens, draft_probs, spec_step, top_p, rand_top_k):
    ...
    perf(lambda: ref_reject_sampling(target_logits, draft_tokens, draft_probs, spec_step, top_p, rand_top_k))  # noqa: F821
    perf(lambda: reject_sampling(target_logits, draft_tokens, draft_probs, spec_step, top_p, rand_top_k))  # noqa: F821
...
def test_magic_reject_sampling(target_logits, draft_tokens, draft_probs, spec_step, top_p, rand_top_k):
    ...
    ref_reject_sampling = RefRejectMagicRejectSampling()
    reject_sampling = MojoMagicRejectSampling()
