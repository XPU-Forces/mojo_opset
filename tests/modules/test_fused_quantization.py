...
@pytest.mark.api("modules.RMSNormQuant")
@pytest.mark.api("modules.LayerNormQuant")
@pytest.mark.api("modules.ResidualAddRMSNormQuant")
@pytest.mark.api("modules.ResidualAddLayerNormQuant")
...
def test_norm(accuracy_backend, shape, dtype, mode):
    ...
    call, reference, inputs = norm_case(shape, dtype, mode, device, impl, True)
    check_norm(call(*inputs), reference(*inputs), mode, dtype)


@pytest.mark.api("modules.MoEDynamicQuant")
@pytest.mark.accuracy
...
def test_moe(accuracy_backend, case, dtype, cpu_counts):
    ...
    call, reference, inputs = moe_case(case, dtype, device, impl, True)
    if cpu_counts:
        inputs = (inputs[0], inputs[1].cpu())
    check_grouped(call(*inputs), reference(*inputs))


@pytest.mark.api("modules.DequantSwiGLUQuant")
@pytest.mark.accuracy
@pytest.mark.parametrize("case", SWIGLU_CASES)
def test_swiglu(accuracy_backend, case):
    impl, target, device = accuracy_backend
    supported(impl, target)
    call, reference, inputs = swiglu_case(case, device, impl, True)
...
def test_fp8_reference(shape, dtype, mode):
    ...
    call, _, inputs = norm_case(shape, dtype, mode, "cpu", "torch_reference", True, fp8=True)
    output = call(*inputs)
...
def test_norm_options(accuracy_backend, mode, affine, symmetric):
    ...
    call, reference, inputs = norm_case(
        (2, 256), torch.float16, mode, device, impl, True, smooth=True, affine=affine, symmetric=symmetric
    )
