...
@pytest.mark.api(
    "functions.paged_prefill_gqa_infer",
    "functions.paged_decode_gqa_infer",
    "functions.paged_prefill_swa_infer",
    "functions.paged_decode_swa_infer",
)
...
def test_paged(accuracy_backend, spec):
    ...
    call, ref, inputs, dtype = paged_case(spec, device, impl, False)
    assert_close(call(*inputs), ref(*inputs), dtype, rtol=2e-2, atol=2e-2)


@pytest.mark.api("functions.paged_prefill_gqa_infer")
@pytest.mark.accuracy
...
@pytest.mark.api("functions.prefill_gqa_infer")
@pytest.mark.accuracy
@pytest.mark.parametrize("case", PREFILL_CASES)
@pytest.mark.parametrize("layout", ["ABAB", "AABB"])
def test_prefill(accuracy_backend, case, layout):
    impl, _, device = accuracy_backend
    call, ref, inputs = prefill_case(case, layout, device, impl, False)
