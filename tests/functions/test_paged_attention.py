...
def paged_case(spec, device, impl):
    ...
    op = partial(getattr(F, name), **config, implementation=impl)
...
def prefill_case(case, layout, device, impl):
    ...
    op = partial(F.prefill_gqa_infer, gqa_layout=layout, implementation=impl)
...
@pytest.mark.parametrize(
    "spec",
    [pytest.param(spec, marks=pytest.mark.api("functions." + spec[0])) for spec in PAGED_CASES],
