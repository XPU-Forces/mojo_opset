...
@pytest.mark.api("functions.linear_cross_entropy_v2")
...
def test_ce_reduction(accuracy_backend, reduction):
    ...
    def run(x, weight, **selection):
        return F.linear_cross_entropy_v2(x, weight, labels, reduction=reduction, **selection)
...
@pytest.mark.api("functions.linear_cross_entropy_v2_and_zloss")
...
def test_ce_zloss(accuracy_backend, calc_acc):
    ...
    def run(x, weight, **selection):
        return F.linear_cross_entropy_v2_and_zloss(x, weight, labels, 0.01, calc_acc=calc_acc, **selection)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for op in ["linear_cross_entropy_v2", "linear_cross_entropy_v2_and_zloss"]
    ],
)
@pytest.mark.bitwise
...
def _ce_call(x, weight, labels, case, implementation):
    return F.linear_cross_entropy_v2_and_zloss(
        x,
        weight,
        labels,
        case[4],
